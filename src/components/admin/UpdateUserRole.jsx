...
const UpdateUserRole = () => {
  ...
  const handleSave = async () => {
    if (!userId || !role) {
      setError('Please fill in both User ID and Role.');
      return;
    }
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setUserId('');
        setRole('');
        setUserInfo(null); // Reset user info after updating role
      }
    } catch (err) {
  ...
  const handleCancel = () => {
    setUserId('');
    setRole('');
    setUserInfo(null); // Clear user info when cancelling
    setError(null);
  };
          ...
          <div className="bg-contentbg h-full shadow-2xl w-2/3 max-xl:w-full p-10 rounded-3xl">
            ...
            <div className="flex flex-col gap-8 mt-6 pb-10 pl-2 border-b-2">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={role} // Editable field
                  onChange={(e) => setRole(e.target.value)} // On change handler for editable role
                  className="border-2 p-2 rounded-lg shadow-2xl h-10 w-80 max-sm:w-full"
                />
              </div>
            </div>
            ...
            {isLoading && <p className="mt-4">Saving...</p>}
          </div>
        </div>
      </div>
    </div>
